...
export class fakeUnidadesEducativas3627667477608 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (let iue = INITIAL; iue < INITIAL + NRO_UNIDADES_EDUCATIVAS; iue++) {
      ...
      _unidadEducativa.id = TextService.textToUuid(`colegio-empaquetado${iue}`);
      _unidadEducativa.nombre = `${fkCompany.companyName()} ::empaquetado:: ${deptos[rndPosition].nombre} ${iue}`;
      _unidadEducativa.codigoSie = iue + 54321;
      ...
      _persona.id = TextService.textToUuid(`persona-empaquetado${iue}`);
      ...
      _usuario.id = TextService.textToUuid(`usuario-empaquetado${iue}`);
      _usuario.usuario = `DIRECTOR-EMPAQUETADO${iue}`;
      ...
      _usuarioRol.id = TextService.textToUuid(`usuarioRol-empaquetado${iue}`);
