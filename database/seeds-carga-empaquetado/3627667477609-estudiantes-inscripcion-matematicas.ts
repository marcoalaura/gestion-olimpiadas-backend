...
export class fakeEstudiantesInscripcionMatematicas3627667477609 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    _medallero.id = TextService.textToUuid(`medallero-matematicas-empaquetado${INITIAL}`)
    ...
    for (let iue = INITIAL; iue < INITIAL + NRO_UNIDADES_EDUCATIVAS; iue++) {
      for (let idx = INITIAL; idx < INITIAL + NRO_ESTUDIANTES; idx++) {
        ...
        _persona.id = TextService.textToUuid(`persona-estudiante-empaquetado${iue}-${idx}`);
        _persona.nombres = `${fkName.firstName()}-${iue}-${idx}`;
        _persona.primerApellido = fkName.lastName();
        _persona.nroDocumento = `7654321${iue}${idx}`;
        ...
        _estudiante.id = TextService.textToUuid(`estudiante-empaquetado${iue}-${idx}`);
        _estudiante.rude = `7654321${iue}${idx}`;
        ...
        _ue.id = TextService.textToUuid(`colegio-empaquetado${iue}`);
