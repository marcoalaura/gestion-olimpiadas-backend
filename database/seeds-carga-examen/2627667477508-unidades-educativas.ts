...
export class fakeUnidadesEducativas2627667477508 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (let iue = INITIAL; iue < INITIAL + NRO_UNIDADES_EDUCATIVAS; iue++) {
      ...
      _unidadEducativa.id = TextService.textToUuid(`colegio-examen${iue}`)
      _unidadEducativa.nombre = `${fkCompany.companyName()} ${deptos[rndPosition].nombre} ${iue}`;
      _unidadEducativa.codigoSie = iue + 5000;
      ...
      _persona.id = TextService.textToUuid(`persona-examen${iue}`);
      ...
      _usuario.id = TextService.textToUuid(`usuario-examen${iue}`);
      _usuario.usuario = `DIRECTOR-EXAMEN${iue}`;
      ...
      _usuarioRol.id = TextService.textToUuid(`usuarioRol-examen${iue}`);
