...
export class estudianteExamenFisica2627667477513 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (let iue = INITIAL; iue < INITIAL + NRO_UNIDADES_EDUCATIVAS; iue++) {
      ...
      for (let idx = INITIAL; idx < INITIAL + NRO_ESTUDIANTES; idx++) {
        ...
        _examen.id = TextService.textToUuid(`estex-online-fisica-examen${iue}-${idx}`);
        // _examen.fechaInicio = new Date('2021-08-12 08:00:00');
        ...
        for (let index = 0; index < NRO_PREGUNTAS; index++) {
          ...
          await queryRunner.query(`
            ...
            VALUES ('${TextService.textToUuid(`estexdet-online-fisica-examen-${iue}-${idx}-${index}`)}', null, null, 'ACTIVO', '${TextService.textToUuid(`estex-online-fisica-examen${iue}-${idx}`)}', '${bancoPreguntas[nro_pregunta].id}', DEFAULT, 'seeders-fake') RETURNING id;
          `);
        ...
        // _examenOffline.id = TextService.textToUuid(`estex-offline-fisica-examen${iue}-${idx}`);
        ...
        //    await queryRunner.query(`
        //      INSERT INTO estudiante_examen_detalle (id, respuestas, puntaje, estado, id_estudiante_examen, id_pregunta, fecha_creacion, usuario_creacion)
        //      VALUES ('${v4()}', null, null, 'ACTIVO', '${TextService.textToUuid(`estex-offline-fisica-examen${iue}-${idx}`)}', '${bancoPreguntasOffline[nro_pregunta_offline].id}', DEFAULT, 'seeders-fake') RETURNING id;
        //    `);
