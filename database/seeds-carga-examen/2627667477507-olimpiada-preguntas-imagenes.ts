...
export class fakePreguntasImagenes2627667477507 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const NRO_IMAGENES = parseInt(process.env.NRO_IMAGENES_HARD) || 200;
    const INDICE_IMAGENES = parseInt(process.env.INDICE_IMAGENES_HARD) || 5000;
    ...
    // PREGUNTAS NIVEL BASICO
      for (let idx = 0; idx < tam; idx++) {
        ...
        _pregunta.idEtapaAreaGrado = TextService.textToUuid(`eag-carga-quimica-examen${INITIAL}`); //'793e2227-770d-5692-8852-d9efdcc6a4d7';
        _pregunta.estado = 'APROBADO';
        _pregunta.usuarioCreacion = 'd5de12df-3cc3-5a58-a742-be24030482d8';
        _pregunta.imagenPregunta = `/${NOMBRE_CARPETA_IMG}/${INICIALES_IMG}-${Math.floor(Math.random() * ((INDICE_IMAGENES + NRO_IMAGENES - 1) - INDICE_IMAGENES + 1)) + INDICE_IMAGENES}.jpg`
  
        await queryRunner.manager.save(_pregunta);
        // preguntas.push(_pregunta);
