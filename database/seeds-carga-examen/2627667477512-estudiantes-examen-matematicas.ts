...
export class estudianteExamenMatematica2627667477512 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (let iue = INITIAL; iue < INITIAL + NRO_UNIDADES_EDUCATIVAS; iue++) {
      ...
      for (let idx = INITIAL; idx < INITIAL + NRO_ESTUDIANTES; idx++) {
        ...
        _examen.id = TextService.textToUuid(`estex-online-matematicas-examen${iue}-${idx}`);
        _examen.fechaInicio = null;
        _examen.fechaConclusion = null;
        _examen.fechaFin = null;
        ...
        for (let index = 0; index < NRO_PREGUNTAS; index++) {
          ...
          await queryRunner.query(`
            ...
            VALUES ('${TextService.textToUuid(`estexdet-online-matematicas-examen-${iue}-${idx}-${index}`)}', null, null, 'ACTIVO', '${TextService.textToUuid(`estex-online-matematicas-examen${iue}-${idx}`)}', '${bancoPreguntas[nro_pregunta].id}', DEFAULT, 'seeders-fake') RETURNING id;
          `);
        ...
        // _examenOffline.id = TextService.textToUuid(`estex-offline-matematicas-examen${iue}-${idx}`);
        ...
        //   await queryRunner.query(`
        //     INSERT INTO estudiante_examen_detalle (id, respuestas, puntaje, estado, id_estudiante_examen, id_pregunta, fecha_creacion, usuario_creacion)
        //     VALUES ('${uuidv4()}', null, null, 'ACTIVO', '${TextService.textToUuid(`estex-offline-matematicas-examen${iue}-${idx}`)}', '${bancoPreguntasOffline[nro_pregunta_offline].id}', DEFAULT, 'seeders-fake') RETURNING id;
        //   `);
