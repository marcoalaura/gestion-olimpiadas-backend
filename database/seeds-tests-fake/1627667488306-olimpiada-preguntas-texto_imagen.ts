...
export class fakePreguntasTextoImagen1627667488306 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // PREGUNTAS NIVEL BASICO
      for (let idx = 0; idx < tam; idx++) {
        ...
        _pregunta.idEtapaAreaGrado = TextService.textToUuid(`eag-sorteo-fisica-${INITIAL}`); //'793e2227-770d-5692-8852-d9efdcc6a4d7';
        _pregunta.estado = 'APROBADO';
        _pregunta.usuarioCreacion = 'seeders-sorteo-fake';
        _pregunta.imagenPregunta = Math.random() < 0.2 ? `/${NOMBRE_CARPETA_IMG}/${INICIALES_IMG}-${Math.floor(Math.random() * ((INDICE_IMAGENES + NRO_IMAGENES - 1) - INDICE_IMAGENES + 1)) + INDICE_IMAGENES}.png` : null;
