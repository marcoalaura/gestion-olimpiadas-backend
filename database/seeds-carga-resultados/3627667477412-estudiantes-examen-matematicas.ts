...
export class estudianteExamenMatematica3627667477412 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (let iue = INITIAL; iue < INITIAL + NRO_UNIDADES_EDUCATIVAS; iue++) {
      ...
      for (let idx = INITIAL; idx < INITIAL + NRO_ESTUDIANTES; idx++) {
        ...
        _examen.id = TextService.textToUuid(`estex-online-matematicas-resultados${iue}-${idx}`);
        ...
        _examen.data = {
          olimpiada: 'Olimpiadas RESULTADOS 2021',
        ...
        for (let index = 0; index < NRO_PREGUNTAS; index++) {
          ...
          await queryRunner.query(`
            ...
            VALUES ('${TextService.textToUuid(`estexdet-online-matematicas-resultados-${iue}-${idx}-${index}`)}', null, null, 'ACTIVO', '${TextService.textToUuid(`estex-online-matematicas-resultados${iue}-${idx}`)}', '${bancoPreguntas[nro_pregunta].id}', DEFAULT, 'seeders-fake') RETURNING id;
          `);
        ...
        _examenOffline.id = TextService.textToUuid(`estex-offline-matematicas-resultados${iue}-${idx}`);
        _examenOffline.fechaInicio = new Date('2021-08-10');
        _examenOffline.fechaFin = new Date('2021-08-30');
        _examenOffline.estadoCargadoOffline = 'PENDIENTE';
        _examenOffline.estado = Status.ACTIVE;
        ...
        _examenOffline.data = {
          olimpiada: 'Olimpiadas RESULTADOS CSV 2021 - Resultados',
          rude: _inscripcion.id,
          distritoEducativo: 'Distrito educativo test',
          gradoEscolar: 'Primero de secundaria',
          estudiante: 'JUAN PINTO ESCOBAR',
        ...
        for (let index = 1; index < NRO_PREGUNTAS; index++) {
          ...
          await queryRunner.query(`
            INSERT INTO estudiante_examen_detalle (id, respuestas, puntaje, estado, id_estudiante_examen, id_pregunta, fecha_creacion, usuario_creacion)
            VALUES ('${TextService.textToUuid(`estexdet-offline-matematicas-resultados-${iue}-${idx}-${index}`)}', null, null, 'ACTIVO', '${TextService.textToUuid(`estex-offline-matematicas-resultados${iue}-${idx}`)}', '${bancoPreguntasOffline[nro_pregunta_offline].id}', DEFAULT, 'seeders-fake') RETURNING id;
          `);
