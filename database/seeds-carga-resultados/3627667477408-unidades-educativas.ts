...
export class fakeUnidadesEducativas3627667477408 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (let iue = INITIAL; iue < INITIAL + NRO_UNIDADES_EDUCATIVAS; iue++) {
      ...
      _unidadEducativa.id = TextService.textToUuid(`colegio-resultados${iue}`);
      _unidadEducativa.nombre = `${fkCompany.companyName()} ::resultados:: ${deptos[rndPosition].nombre} ${iue}`;
      _unidadEducativa.codigoSie = iue + 99999;
      ...
      _persona.id = TextService.textToUuid(`persona-resultados${iue}`);
      ...
      _usuario.id = TextService.textToUuid(`usuario-resultados${iue}`);
      _usuario.usuario = `DIRECTOR-RESULTADOS-${iue}`;
      ...
      _usuarioRol.id = TextService.textToUuid(`usuarioRol-resultados${iue}`);
