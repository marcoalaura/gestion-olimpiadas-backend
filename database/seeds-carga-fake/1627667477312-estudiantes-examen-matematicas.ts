...
export class estudianteExamenMatematica1627667477312 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    _eag.id = TextService.textToUuid(`eag-carga-matematicas-${INITIAL}`);
    ...
    // const estudiante_examen_detalle = [];
    ...
    for (let iue = INITIAL; iue < INITIAL + NRO_UNIDADES_EDUCATIVAS; iue++) {
      ...
      for (let idx = INITIAL; idx < INITIAL + NRO_ESTUDIANTES; idx++) {
        ...
        _inscripcion.id = TextService.textToUuid(`inscripcion-matematicas-${iue}-${idx}`);
        // _inscripcion.id = TextService.textToUuid(`inscripcion-matematicas-${iue}-${idx}`);


        const _examen = new EstudianteExamen();
        // _examen.id = TextService.generateUuid();
        _examen.id = TextService.textToUuid(`estex-online-matematicas-${iue}-${idx}`);
        ...
        for (let index = 0; index < NRO_PREGUNTAS; index++) {
          ...
          await queryRunner.query(`
            ...
            VALUES ('${TextService.textToUuid(`estexdet-online-matematicas-fake-${iue}-${idx}-${index}`)}', null, null, 'ACTIVO', '${TextService.textToUuid(`estex-online-matematicas-${iue}-${idx}`)}', '${bancoPreguntas[nro_pregunta].id}', DEFAULT, 'seeders-fake') RETURNING id;
          `);
        ...
        _examenOffline.id = TextService.textToUuid(`estex-offline-matematicas-${iue}-${idx}`);
        ...
        estudiante_examen.push(_examenOffline);
        ...
        for (let index = 1; index < NRO_PREGUNTAS; index++) {
          ...
          await queryRunner.query(`
            INSERT INTO estudiante_examen_detalle (id, respuestas, puntaje, estado, id_estudiante_examen, id_pregunta, fecha_creacion, usuario_creacion)
            VALUES ('${TextService.textToUuid(`estexdet-offline-matematicas-fake-${iue}-${idx}-${index}`)}', null, null, 'ACTIVO', '${TextService.textToUuid(`estex-offline-matematicas-${iue}-${idx}`)}', '${bancoPreguntasOffline[nro_pregunta_offline].id}', DEFAULT, 'seeders-fake') RETURNING id;
          `);
