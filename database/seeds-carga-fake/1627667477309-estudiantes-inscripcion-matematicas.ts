...
export class fakeEstudiantesInscripcionMatematicas1627667477309 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    _medallero.id = TextService.textToUuid(`medallero-matematicas-${INITIAL}`)
    ...
    for (let iue = INITIAL; iue < INITIAL + NRO_UNIDADES_EDUCATIVAS; iue++) {
      ...
      for (let idx = INITIAL; idx < INITIAL + NRO_ESTUDIANTES; idx++) {
        ...
        _estudiante.id = TextService.textToUuid(`estudiante-${iue}-${idx}`);
        ...
        _ue.id = TextService.textToUuid(`colegio-${iue}`);
