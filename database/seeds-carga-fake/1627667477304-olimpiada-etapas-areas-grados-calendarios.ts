...
export class fakeEtapaAreaGradoCalendario1627667477304 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const etapasItems = [{
      id: TextService.textToUuid(`etapa1-carga-${INITIAL}`),
      nombre: `Etapa 1 para pruebas de carga - ${INITIAL}`,
      ...
      estado: Status.DESCARGA_EMPAQUETADOS,
    ...
    const eagsItems = [
      {
        id: TextService.textToUuid(`eag-carga-matematicas-${INITIAL}`),
        total_preguntas: 5,
        ...
        id_etapa: TextService.textToUuid(`etapa1-carga-${INITIAL}`),
        id_grado_escolar: 'c9930379-5dc1-556f-847c-802dd9d1d9b6',// PRIMERO SECUNDARIA
        id_medallero: TextService.textToUuid(`medallero-matematicas-${INITIAL}`)
      },
      {
        id: TextService.textToUuid(`eag-carga-fisica-${INITIAL}`),
        total_preguntas: 14,
        ...
        id_medallero: TextService.textToUuid(`medallero-fisica-${INITIAL}`)
      },
      {
        id: TextService.textToUuid(`eag-carga-quimica-${INITIAL}`),
        total_preguntas: 14,
        preguntas_curricula: 6,
        preguntas_olimpiada: 8,
        puntos_pregunta_curricula: 40,
        ...
        id_medallero: TextService.textToUuid(`medallero-quimica-${INITIAL}`)
      },
    ];
